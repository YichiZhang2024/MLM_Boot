...
1. Fit a two-level model to data and obtain the estimates for fixed effects and random effects.
2. Draw a sample of size $J$, $\{j^*_k, k = 1,...,J\}$, with replacement from the identifier of level-2 units.
3. Draw a sample of entire cases, ${(y_{ik}^*, \bv{X}_{ik}^*, \bv{Z}_{ik}^*)}$, where $i = 1,...,n_k^*$, with replacement from the selected level-2 unit $j = j^*_k$.
4. Refit the two-level model to the bootstrap data and obtain parameter estimates.
5. Repeat steps 2-4 $B$ times and compute the bias-corrected estimates and standard errors.
...
1. Fit a two-level model to data and obtain the estimates for fixed effects and random effects.
2. Draw one entire level-2 unit, ${(\bv{y}_{j}, \bv{X}_{j}, \bv{Z}_{j})}$, with replacement.
3. Draw a sample of level-1 observations with size $n_j$ from the selected level-2 unit.
3. Repeat steps 2-3 $J$ times.
