...
Tables\ \@ref(tab:comp1)-\@ref(tab:comp4) summarize the standard errors and CIs for all parameters with the illustrated bootstrap approaches. The tables also include the uncorrected estimates, standard errors from the `lme4` model output, as well as the confidence intervals based on profile likelihood using `confint(mod_f)`. Note that with non-bootstrap methods, it is generally difficult to obtain the confidence intervals for ICC and $R^2$ and the standard errors for random effects, ICC, and $R^2$. Recall that parametric bootstrap assumes normality; residual bootstrap relaxes the normality assumption but still relies on the homoscedasticity assumption; wild bootstrap relaxes the homoscedasticity assumption but relies on the normality assumption; cases bootstrap relaxes both assumptions but requires more time to perform with lower computational efficiency. Since the intelligence variable violates both the normality and homoscedasticity assumptions, we expect to see differences in the standard errors and CIs between methods for the fixed effects related to intelligence. 

For the school-level fixed effects, the CIs are similar across methods, except for the school-mean intelligence ($\gamma_{01}$). The standard errors are also similar across methods, except for the grand intercept ($\gamma_{00}$) and the fixed effect of school-mean intelligence ($\gamma_{01}$). The cases bootstrap with resampling at both levels shows the most drastic differences with larger standard errors and wider CIs. For the student-level fixed effects, the standard errors and CIs are similar across methods, except for the fixed effect of student-level intelligence ($\gamma_{10}$) and the interaction between the student-level intelligence and socio-economic status ($\gamma_{30}$). Similarly, the cases bootstrap with resampling at both levels gives larger standard errors and wider CIs.

For the random effects, the confidence intervals based on profile likelihood are drastically different from the confidence intervals from the bootstrap methods. For the $R^2$ and ICC, all methods yield similar standard errors and CIs. For the ICC specifically, cases bootstrap with resampling at both levels tend to give internvals of the shortest length with the normal and basic CIs. <!-- ML: you mean "narrower" CI? lower CI doesn't seem to make sense WT: sounds good, thanks-->
