...
In this section, we illustrate three bootstrap approaches (i.e., parametric, residual, and cases) with the *bootmlm* R package and the wild bootstrap with the *lmeresampler* R package on an empirical example. In particular, we will obtain bias-corrected estimates, standard errors, and CIs for the fixed effects, random effects, intraclass correlation (ICC), and $R^2$ [@nakagawa2013]. <!-- ML: Also mention bias-corrected estimates? WT: done. Thanks-->
