...
In this section, we illustrate three bootstrap approaches (i.e., parametric, residual, and cases) with the *bootmlm* R package and the wild bootstrap with the *lmeresampler* R package on an empirical example. In particular, we will obtain bias-corrected estimates, standard errors, and CIs for the fixed effects, intraclass correlation (ICC), and $R^2$. <!-- ML: Also mention bias-corrected estimates? WT: done. Thanks-->
...
We first define two functions: `fixef_rsq()` takes the full model to compute the fixed effects and $R^2$, and `icc()` takes the null model to compute ICC. `fixef_rsq()` yields estimates for nine parameters, including the eight fixed effects ($\gamma_{00}, \gamma_{01}, \gamma_{02}, \gamma_{03}, \gamma_{10}, \gamma_{20}, \gamma_{30}, \gamma_{40}$) and $R^2$. In `icc()`, we compute the ICC and additionally its asymptotic sampling variance, which will be used to construct the studentized/bootstrap-*t* CI. 
...
<!-- ML: boot.ci() supports five. Would you clarify that studentized and BCa require additional input? WT: sounds good. --> As `boo_par_fr` consists of the bootstrap samples for nine parameters, `lapply()` allows us to compute CIs for all of them simultaneously. 
...
The `boot::boot.ci()` function also supports five ways to compute CI, including normal, basic, percentile, studentized, and BCa. Again, the influence values are needed to compute CIs with BCa, and we use the `lapply()` function to compute the CIs for all nine statistics defined in `fixef_rsq()` in one command.  
...
```{r echo=FALSE}
tab1_cap <- "
Bias-Corrected Estimatoes, Standard Errors, and Confidence Intervals for School-Level Fixed Effects
"
tab2_cap <- "
Bias-Corrected Estimatoes, Standard Errors, and Confidence Intervals for Student-Level Fixed Effects
"
tab3_cap <- "
Bias-Corrected Estimatoes, Standard Errors, and Confidence Intervals for Random Effects
"
tab4_cap <- "
Bias-Corrected Estimatoes, Standard Errors, and Confidence Intervals for $R^2$ and ICC
"
```
...
compare_boo %>%
  ...
  knitr::kable(
    booktabs = TRUE, 
    escape = FALSE, 
    col.names = c("", "Bootstrap Type", "$\\tau^2_{00}$", 
...
For $R^2$ and ICC, all methods yield similar standard errors and CIs. Cases bootstrap with resampling at both levels tend to give a narrower normal and basic CIs than other CI methods for ICC. <!-- ML: you mean "narrower" CI? lower CI doesn't seem to make sense WT: sounds good, thanks-->
...
Here we demonstrate how to obtain CIs for $R^2$ with the logit transformation ($f[x]$ = $x / [1 - x]$). In `boot::boot.ci()`, we additionally provide three inputs: (a) the logit function, `qlogis`, (b) the first derivative of the logit function function `function(x) 1 / (x - x^2)`, and (c) the inverse of the logit function, `plogis`. As $R^2$ is the ninth parameter in our defined function `fixef_rsq()` for bootstrapping, we specify `index = 9` to compute the CI for $R^2$. Below is the syntax for applying logit transformation for the bootstrap samples of $R^2$ with parametric, residual, and cases bootstrap. 
