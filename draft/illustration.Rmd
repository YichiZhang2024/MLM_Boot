...
For ICC, we further demonstrate how to construct the studentized CI. Since `icc()` computes both the estimate and asymptotic sampling variance of ICC, `boo_par_icc` contains the bootstrap samples of both estimators, i.e., $\hat \rho^*$ and $\hat{\text{Var}}(\hat \rho^*)$. We specify the argument `type = "stud"` in `boot.ci()`, which takes the bootstrap samples of $\hat \rho^*$ and $\hat{\text{Var}}(\hat \rho^*)$ to construct the studentized CI. 
