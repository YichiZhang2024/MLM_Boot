...
For the fixed effects, random effects, and $R^2$, we exemplify four ways to construct CIs with `boot::boot.ci()` with residual bootstrap methods: normal, basic, percentile, and additionally, BCa. For the ICC, we also illustrate the studentized CI. As discussed in Section III, the influence values are needed to compute CIs with BCa and can be obtained using the function `empinf_mer()` in *bootmlm*. This function requires the inputs of the fitted model (`mod_f` and `mod_0`), the defined function that computes the parameter estimate(s) (`eff_rsq()` and `icc()`), and the index of the statistic in the output of the defined function. As `eff_rsq()` yields 13 statistics, we can use `lapply()` to conveniently compute the influence values for all of them at once. 
