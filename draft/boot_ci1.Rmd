...
If normality holds, then the common approximate $(1-\alpha) \times 100\%$ symmetric CI is $$[\hat \theta - z_{1-\frac{1}{2}\alpha}\hat{se}_N, \hat \theta + z_{1-\frac{1}{2}\alpha}\hat{se}_N],$$ where $\hat \theta$ is the estimated parameter, and $\hat se_N$ is the sample standard error of $\hat \theta$. For bootstrap normal confidence interval, $\hat{se}_N$ is replaced with $\hat{se}^*$ [@vanderleeden2008],<!-- ML: I think Winnie used a different notation for bootstrap SE --> the bootstrap estimator of the standard deviation of $\hat \theta$, so the bootstrap normal CI can be constructed as $$[\hat \theta - z_{1-\frac{1}{2}\alpha}\hat{se}^*, \hat \theta + z_{1-\frac{1}{2}\alpha}\hat{se}^*].$$
