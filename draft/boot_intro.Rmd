...
However, suppose that our data has a skewed distribution and violates the normality assumption. To account for nonnormality, we perform bootstrapping in the following procedure [@wilcox2017; @efron1993]:
...
Based on the bootstrap samples, we can estimate the sample mean by taking the average of the bootstrap sample means, $\bar X^*=\frac{1}{B}\sum_b^BX^*_b$, and the standard error of the mean by taking the average of the bootstrap standard errors, $se^*=\frac{1}{B}\sum_b^Bse^*_b$. The bias of the sample mean, $\bar X$, is estimated by the bias of $\bar X^*$ [@efron1993; @vanderleeden2008], given by
\begin{equation}
...
\begin{equation}
[\bar X^* - t_{1 - \alpha/2}^{*} se^{*}, \bar X^* + t_{1 - \alpha/2}^{*} se^{*}]
\end{equation}
